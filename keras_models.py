...
def dilation_pool(x, window_shape, strides, dilation_rate):
...
def deep_model_mimic(n_chans, input_time_length, n_classes, n_filters_time=25, n_filters_spat=25, filter_time_length=10,
    ...
    model.add(Conv2D(filters=n_classes, kernel_size=(1, 2), strides=(1, 1)))
...
def convert_to_dilated(model):
    ...
    # mean = Lambda(mean_layer)(model.layers[-2].output)
    # flat = Flatten(mean)
    # model = Model(input=model.input, output=[flat])
