...
import EEGNAS.utilities.monitors
from EEGNAS.utilities.misc import chunks
...
def ensemble_fitness(weighted_population):
    ...
    for iteration in range(global_vars.get('ensemble_iterations')):
        pop_indices = list(range(global_vars.get('pop_size')))
        random.shuffle(pop_indices)
        ensembles = list(chunks(pop_indices, global_vars.get('ensemble_size')))
...
def one_subject_one_ensemble_fitness(weighted_population, ensemble, str_prefix=''):
    ...
    ensemble_fit = getattr(EEGNAS.utilities.monitors, f'{global_vars.get("ga_objective")}_func')(pred_labels, ensemble_targets)
    return ensemble_fit
...
def cross_subject_shared_fitness(weighted_population):
    for item in weighted_population:
        fitness = item[f'val_{global_vars.get("ga_objective")}']
        subject_array = range(1, global_vars.get('num_subjects') + 1)
        fitness_vector = [item[f'{i}_val_{global_vars.get("ga_objective")}'] for i in subject_array]
        denominator = 1
        dists = []
        for pop in weighted_population:
            pop_fitness = [pop[f'{i}_val_{global_vars.get("ga_objective")}'] for i in range(1, global_vars.get('num_subjects') + 1)]
            dists.append(pdist([fitness_vector, pop_fitness])[0])
        dists_norm = [float(i)/max(dists) for i in dists]
        for dist in dists_norm:
            if dist < global_vars.get('min_dist'):
                denominator += 1-dist
...
def normal_fitness(weighted_population):
    for pop in weighted_population:
        pop['fitness'] = pop[f'val_{global_vars.get("ga_objective")}']
